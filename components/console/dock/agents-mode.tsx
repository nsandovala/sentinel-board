...
export function AgentsMode({ expanded: _expanded }: AgentsModeProps) {
  return (
    <div className="flex flex-col gap-3 px-4 py-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 rounded-md border border-border/30 bg-background/40 px-2 py-0.5 text-[10px] text-muted-foreground">
          ...
          Stream no conectado · se enlazará con /api/stream en Fase 2
        </div>
      </div>

      <ul className="flex flex-col divide-y divide-border/25 rounded-md border border-border/30 bg-background/30">
        {AGENTS.map((agent) => (
          <li
            key={agent.key}
